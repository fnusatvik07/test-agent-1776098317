...
CORS_ORIGINS = [
    ...
    f"http://localhost:8012",
    f"http://localhost:8013",
    f"http://localhost:8014",
    f"http://127.0.0.1:8012",
    f"http://127.0.0.1:8013",
    f"http://127.0.0.1:8014"
]
...
- When you lack sufficient information, use web search to find current and relevant data
...
4. If the question requires current information or verification, use web search and web fetch tools to gather data
5. Synthesize the information and deliver a clear, professional response
...
- Provide brief summaries for complex topics, with option to elaborate if requested""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8011",
        "frontend_url": f"http://localhost:8012",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8011")
    print(f"[FRONTEND] Frontend URL: http://localhost:8012")
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8011/docs")
    print(f"[HEALTH] Health: http://localhost:8011/health")
